...
namespace AssetFlow.API.Middleware
{
    public class ExceptionHandlingMiddleware
    {
        ...
        public ExceptionHandlingMiddleware(RequestDelegate next, ILogger logger, IOptions<SystemSettings> systemSettings)
        {
            this.logger = logger;
            this.next = next;
        ...
        private Task HandleExceptionAsync(HttpContext context, Exception exception, string exMsg)
        {
            ...
            var json = JsonConvert.SerializeObject(new ResponseDto<string>()
            {
                ...
                Message = message,
                Exception = systemSettings.ShowExMsg ? exMsg : string.Empty
            },
